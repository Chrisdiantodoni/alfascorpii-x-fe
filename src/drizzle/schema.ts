...
export const files = pgTable(
	...
	(table) => [
		index().using(
			"btree",
			table.fileableType.asc().nullsLast().op("text_ops"),
			table.fileableId.asc().nullsLast().op("text_ops"),
		),
	],
);
...
export const pages = pgTable(
	...
	(table) => [unique("pages_slug_unique").on(table.slug)],
);
...
export const seoMetadata = pgTable(
	...
	(table) => [
		index().using(
			"btree",
			table.seoableType.asc().nullsLast().op("text_ops"),
			table.seoableId.asc().nullsLast().op("text_ops"),
		),
	],
);
...
export const inquiries = pgTable(
	...
	(table) => [
		check(
			...
			sql`(status)::text = ANY ((ARRAY['new'::character varying, 'followed_up'::character varying, 'closed'::character varying])::text[])`,
		),
	],
);
...
export const banners = pgTable(
	...
	(table) => [
		index().using(
			"btree",
			table.bannerableType.asc().nullsLast().op("text_ops"),
			table.bannerableId.asc().nullsLast().op("text_ops"),
		),
	],
);
...
export const personalAccessTokens = pgTable(
	...
	(table) => [
		...
		index().using(
			"btree",
			table.tokenableType.asc().nullsLast().op("text_ops"),
			table.tokenableId.asc().nullsLast().op("text_ops"),
		),
...
export const menus = pgTable(
	...
	(table) => [unique("menus_location_unique").on(table.location)],
);
...
export const modelHasPermissions = pgTable(
	...
	(table) => [
		index("model_has_permissions_model_id_model_type_index").using(
			"btree",
			table.modelUuid.asc().nullsLast().op("bpchar_ops"),
		...
		primaryKey({
			columns: [table.permissionId, table.modelType, table.modelUuid],
...
export const modelHasRoles = pgTable(
	...
	(table) => [
		index("model_has_roles_model_id_model_type_index").using(
			"btree",
			table.modelUuid.asc().nullsLast().op("bpchar_ops"),
			table.modelType.asc().nullsLast().op("text_ops"),
		),
		...
		primaryKey({
			columns: [table.roleId, table.modelType, table.modelUuid],
...
export const relatedProducts = pgTable(
	...
	(table) => [
		...
		primaryKey({
			columns: [table.productId, table.relatedProductId],
			name: "related_products_pkey",
		}),
		unique("related_products_product_id_related_product_id_unique").on(
			table.productId,
